...
const Trading = ({
  ...
  useEffect(() => {
    const liveTransaction = JSON.parse(localStorage.getItem("LIVE_TRANSACTION"))
    setLiveTransaction(liveTransaction)
                  ...
                  <ul>
                    ...
                    <li>
                      <div className="item">
                        ...
                        <h3 className="title">
                          {activeNetwork?.walletAddress == undefined ? "Select" : shortAddress(activeNetwork.walletAddress)}
                    ...
                    <li>
                      <div className="item">
                        ...
                        <h3 className="title">
                          {activeNetwork?.privateKey == undefined ? "Select" : shortAddress(activeNetwork.privateKey)}
                    ...
                    <li>
                      <div className="item">
                        ...
                        <h3 className="title">
                          {activeNetwork?.rpcUrl == undefined ? "Select" : shortAddress(activeNetwork.rpcUrl)}
              ...
              <div>
                {
                  userMembership !== "notMember" ? (
                    <>
                      {
                        active ? (
                          <a href="" onClick={() => stopTrading()} className='techwave_fn_button'>Stop Bot</a>
                        ) : (
                          <a 
                           onClick={(e) => (e.preventDefault(),trading(activeNetwork, tradeToken), setActive(true),notifySuccess("Transaction Started..."))} 
                           className='techwave_fn_button'>Start Trading</a>
                        )
                      }
                    </>
                  ) : (
                    <a
                      onClick={() => setActiveComponent("Pricing")} className='techwave_fn_button'>
                      Buy Membership
                    </a>
                              ...
                              liveTransaction?.map((transaction,index)=>(
                                <div key={index} className='pricing__item_list'>
                                  <div className="pricing__item_list_item">
                                    ...
                                    <p className="desc">C_Amt:{transaction.currentRate}</p>
                                  </div>
                                </div>
                              ))
                            }
                            ...
                            liveTransaction?.map((transaction,index)=>(
                              <div key={index} className='pricing__fields'>
                                <div className="new_flex">
                                  ...
                                  <div className="item_col">
                                    <span onClick={()=>navigator.clipboard.writeText(transaction.transactionHash)} className="option_text">
