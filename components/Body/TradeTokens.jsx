...
const TradeTokens = () => {
  ...
  const onHandleSearch=(value)=>{
    const filterTokens=tokens?.filter(({network})=>network.toLowerCase().includes(value.toLowerCase()))
                        ...
                        <div className="pricing__item_list">
                          {
                            tokens?.map((token,index)=>(
                              <div onClick={(()=>setTradeToken(token),selectTokenPair())} className="pricing__item_list_item" key={index}>
                                <h4  className='title'>
                                  {token.network} 
                  ...
                  <div className="pricing__content">
                    <div className="pricing__heading">
                      ...
                      <div className="pricing__fields">
                          {tokens?.map((token,index)=>(
                            <div onClick={()=>(setTradeToken(token),selectTokenPair(),setActive(index+1))}  className={`item_row ${active==index+1? "pricing__heading":"" }`}>
